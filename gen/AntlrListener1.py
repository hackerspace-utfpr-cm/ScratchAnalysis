...
class AntlrListener(ParseTreeListener):
    def __init__(self):
        ...
        self.receiveRecord = []  # 记录所有接收消息（重复出现的字符串依然记录）
        self.oneSenderToManyReceiver = 0  # 记录 某个变量是多个脚本的入参
        self.oneReceiverToManySender = 0  # 记录 某个脚本有多个入参变量
        self.countAndOperate_flag = False  # 判断是否开始记录countAndOperate，当进入cblock_doRepeat，cblock_doUntil，cblock_doIfElse，cblock_doIF,cblock_doWaitUntil,cblock_doForever模块时置为True，离开时置为False
        self.countAndOperate = 0  # 记录 进入语法树中遇到'"&"'操作符的个数，遇到时计数加一，离开语法树时计数减一
    ...
    def create_blockCount(self):  # 统计脚本总数和角色总数
        ...
        self.blockCount['oneSenderToManyReceiver'] = self.oneSenderToManyReceiver  # 记录 某个变量是多个脚本的入参 的个数
        self.blockCount['oneReceiverToManySender'] = self.oneReceiverToManySender  # 记录 某个变量是多个脚本的入参 的个数
    ...
    def exitJson(self, ctx):
        # 是否有不匹配的广播
        if (
                    self.whdrop_count >= 2 or self.whrecive_count >= 2 or self.whsensor_count >= 2) and self.Parallelism_score < 3:
        ...
        if len(self.broadcastlist) != 0:
            print("broadcastlistLen:" + str(len(self.broadcastlist)))
            # print(self.broadcastlist[0].encode('utf-8'))
            # print(isinstance(self.broadcastlist[0],utf-8))
            ...
            # print(s.encode('gbk'))
            # print(s1.encode('utf-8').decode('unicode-escape'))

        if len(self.receivelist) != 0:
            print("receivelistLen:" + str(len(self.receivelist)))

        if len(self.receiveRecord) != 0:
            print("receiveRecordLen:" + str(len(self.receiveRecord)))

        for r in self.broadcastlist:
            if self.receiveRecord.count(r) > 1:
                self.oneSenderToManyReceiver += 1
            ...
            if ctx_STRING_Text == '"objName"':
                if ctx.value().STRING().getText() != '"Stage"' and ctx.value().STRING().getText() != '"舞台"':
                    ...
                    if self.sprits_count > 1 and self.scripts_count > 1 and self.ap_score == 0:
                        self.ap_score = 1
                # 是否存在无意义命名
                if ctx.value().STRING().getText().find('Sprite') > 0 or ctx.value().STRING().getText().find(
                        '角色') > 0:
    ...
    def enterValue(self, ctx):
        ...
        # 评分标准1-3
        if (
                    ctx_Text == '"whenCloned"' or ctx_Text == '"createCloneOf"' or ctx_Text == '"deleteClone"') and self.deadcode_flag == False:
            self.clone_count += 1
        ...
        if '"whenClicked"' == ctx_Text and self.deadcode_flag == False:
            ...
            if whcl_meaning >= 1:
                whenclick_name = \
                ctx.parentCtx.parentCtx.parentCtx.parentCtx.parentCtx.parentCtx.children[1].children[2].getText()
        ...
        motions = {'"forward:"', '"turnRight:"', '"turnLeft:"', '"heading:"', '"pointTowards:"', '"gotoX:y:"',
                   '"gotoSpriteOrMouse:"', '"glideSecs:toX:y:elapsed:from:"', '"changeXposBy:"', '"xpos:"',
                   '"changeYposBy:"', '"ypos:"', '"bounceOffEdge"', '"setRotationStyle"'}
        if ctx_Text in motions and self.deadcode_flag == False:
            self.motionnum += 1;
            # print(ctx_Text)

        lookikes = {'"say:duration:elapsed:from:"', '"say:"', '"think:duration:elapsed:from:"', '"think:"',
                    '"show"', '"hide"',
                    '"lookLike:"', '"nextCostume"', '"startScene"', '"changeGraphicEffect:by:"',
                    '"setGraphicEffect:to:"', '"filterReset"', '"changeSizeBy:"', '"setSizeTo:"', '"comeToFront"',
                    '"goBackByLayers:"'}
        ...
        sensorlist = {'"keyPressed:"', '"mousePressed"', '"mouseX"', '"mouseY"', '"soundLevel"',
                      '"senseVideoMotion"', '"setVideoState"', '"distanceTo:"', '"color:sees:"', '"touchingColor:"',
                      '"touching:"', '"timer"', '"getAttribute:of:"', '"timeAndDate"', '"timestamp"',
                      '"getUserName"'}
        ...
        if ctx_Text in operatelist and self.deadcode_flag == False:
            ...
            # print(ctx_Text)
        if ctx_Text == '"&"' and self.countAndOperate_flag == True:
            self.countAndOperate += 1
            print(ctx_Text)
            print(self.countAndOperate)
    ...
    def enterCondition(self, ctx):
        print('Condition')
        self.countAndOperate_flag = True
        print("countAndOperate_flag ="+str(self.countAndOperate_flag))



    # Exit a parse tree produced by AntlrParser#condition.
    def exitCondition(self, ctx):
        self.countAndOperate_flag = False

        if self.countAndOperate > 1:
            self.oneReceiverToManySender += 1
            print("self.oneReceiverToManySender:"+ str(self.oneReceiverToManySender))

        self.countAndOperate = 0
        print("countAndOperate_flag =" + str(self.countAndOperate_flag))
        pass
    ...
    def enterCblock_whenIReceive(self, ctx):
        ...
        # 判断是是否已经收到过该内容
        self.receiveRecord.append(receivecontent) #保存出现的所有的接收信息
